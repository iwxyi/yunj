...
*/ ?><?php // 宏定义
	...
	define("MySQL_database", "yunj");
	
	define("NS", 10); // 10秒钟来回连接时间
...
?><?php // 安全验证操作
	...
	{	
		...
		if ($version < 87 || ($version > 1000 && $version < 1043))
		{
			die("<state>Old</state>");
		}
...
?><?php // 各种取值操作
	...
	function format_input($s, $blank = 0) // 格式化输入内容
	{
		...
		$s = str_replace("'", "''", $s); // 防SQL注入：单个单引号可能会变成两个单引号
		// $s = htmlspecialchars($s); // 防HTML注入：非浏览器显示会变成实体编码
		return $s;
	}
